...
export async function userTriggeredScreen(
	...
	//const pres=0.90;
	const combinationsArraySize = Math.round(
		Object.keys(gramCounts).length * pres
	);
	console.log(`grams are: ${Object.keys(gramCounts).map(
		(key, index) => key
	)} (${Object.keys(gramCounts).length}),\\\
...
function matchLevenshtein(inOne: string, inTwo: string) {
	const one = inOne.toLowerCase(),
		two = inTwo.toLowerCase();
